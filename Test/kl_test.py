...
def calculate_kl(noise, num_samples, noise_color, noise_var, sub_dir, pdf_type='std',
    ...
    for n in range(0, num_samples-step, step):
        # print(n+ (step * 2))
        if est_type == 'kl':
            kl1 = kl.kl_std(noise[n:n + step], noise[n + step: n + (step * 2)], n_bins, pdf_type)
            kl2 = kl.kl_std(noise[n + step: n + (step * 2)], noise[n:n + step], n_bins, pdf_type)
            result.append(np.where(kl1 > kl2, kl1, kl2))
            # result.append(kl1)
        ...
        if ((calc_pdf and (transition_start in range(n + step, n + (step * 2))))
                or (calc_pdf and transition_start == 0 and transition_end == 0)):
            pdf1, pdf2, edges = kl.estimate_pdf(noise[n:n + step],
                                                noise[n + step: n + (step * 2)], n_bins, pdf_type)
            calc_pdf = False
    ...
    if pdf_type == 'std':
        ax[0][1].bar(edges[:-1], pdf1, width=np.diff(edges), edgecolor="black", align="edge")
        ax[0][1].set_title("PDF Histograms for Block 1")
        ax[0][1].set_xlabel("Noise Magnitude")
        ax[0][1].set_ylabel("Probability")
        ax[0][1].grid(True, which='both', ls='-')

        ax[1][0].bar(edges[:-1], pdf2, width=np.diff(edges), edgecolor="black", align="edge")
        ax[1][0].set_title("PDF Histograms for Block 2")
        ax[1][0].set_xlabel("Noise Magnitude")
        ax[1][0].set_ylabel("Probability")
        ax[1][0].grid(True, which='both', ls='-')

    elif pdf_type == 'fft':
        ax[0][1].semilogx(20 * np.log10(pdf1))
        ax[0][1].set_title("Block 1 FFT")
        ax[0][1].set_xlabel("Frequency")
        ax[0][1].set_ylabel("Magnitude (dB)")
        ax[0][1].grid(True, which='both', ls='-')

        ax[1][0].semilogx(20 * np.log10(pdf2))
        ax[1][0].set_title("Block 2 FFT")
        ax[1][0].set_xlabel("Frequency")
        ax[1][0].set_ylabel("Magnitude (dB)")
        ax[1][0].grid(True, which='both', ls='-')


    ax[1][1].plot(eq_samples, result)
    if est_type == 'kl':
        ax[1][1].set_ylabel('KL Divergence')
        ax[1][1].set_title('Standard KL Divergence')
    elif est_type == 'wasserstein':
        ax[1][1].set_ylabel('Wasserstein Distance')
        ax[1][1].set_title('Standard Wasserstein Distance')
    elif est_type == 'jensen-shannon':
        ax[1][1].set_ylabel('JS Divergence')
        ax[1][1].set_title('Standard Jensen-Shannon Divergence')
    ...
    ax[2][0].plot(eq_samples, kl_blocks)
    if est_type == 'kl':
        ax[2][0].set_ylabel('KL Divergence')
        ax[2][0].set_title('KL Divergence Between Blocks')
    elif est_type == 'wasserstein':
        ax[2][0].set_ylabel('Wasserstein Distance')
        ax[2][0].set_title('Wasserstein Between Blocks')
    elif est_type == 'jensen-shannon':
        ax[2][0].set_ylabel('JS Divergence')
        ax[2][0].set_title('JS Divergence Between Blocks')
    ...
    ax[2][1].plot(eq_samples, kl_prog)
    if est_type == 'kl':
        ax[2][1].set_ylabel('KL Divergence')
        ax[2][1].set_title('KL Divergence Progression Between Blocks')
    elif est_type == 'wasserstein':
        ax[2][1].set_ylabel('Wasserstein Distance')
        ax[2][1].set_title('Wasserstein Progression Between Blocks')
    elif est_type == 'jensen-shannon':
        ax[2][1].set_ylabel('JS Divergence')
        ax[2][1].set_title('JS Divergence Progression Between Blocks')
    ax[2][1].set_xlabel('Block Start Sample')
    ...
    ax[3][0].plot(eq_samples, kl_inv_blocks)
    if est_type == 'kl':
        ax[3][0].set_ylabel('KL Divergence')
        ax[3][0].set_title('KL Divergence Between Inverted Blocks')
    elif est_type == 'wasserstein':
        ax[3][0].set_ylabel('Wasserstein Distance')
        ax[3][0].set_title('Wasserstein Between Inverted Blocks')
    elif est_type == 'jensen-shannon':
        ax[3][0].set_ylabel('JS Divergence')
        ax[3][0].set_title('JS Divergence Between Inverted Blocks')
    ax[3][0].set_xlabel('Window Start Sample')
    ax[3][0].grid(True, which='both', ls='-')


    if transition_start != 0 and transition_end != 0:
        ax[3][0].axvline(x=transition_start, color='r', linestyle='dashed')
        ax[3][0].axvline(x=transition_end, color='r', linestyle='dashed')
    ...
    ax[3][1].plot(eq_samples, kl_inv_weight)
    if est_type == 'kl':
        ax[3][1].set_ylabel('KL Divergence')
        ax[3][1].set_title('KL Divergence Accumulating Inverted Blocks')
    elif est_type == 'wasserstein':
        ax[3][1].set_ylabel('Wasserstein Distance')
        ax[3][1].set_title('Wasserstein Accumulating Inverted Blocks')
    elif est_type == 'jensen-shannon':
        ax[3][1].set_ylabel('JS Divergence')
        ax[3][1].set_title('JS Divergence Accumulating Inverted Blocks')
    ax[3][1].set_xlabel('Window Start Sample')
    ax[3][1].grid(True, which='both', ls='-')

    if transition_start != 0 and transition_end != 0:
        ax[3][1].axvline(x=transition_start, color='r', linestyle='dashed')
        ax[3][1].axvline(x=transition_end, color='r', linestyle='dashed')
    ...
    if est_type == 'kl':
        ...
        ax[1].set_title('Wasserstein Distance with Sliding Window')
    elif est_type == 'jensen-shannon':
        ax[1].set_ylabel('JS Divergence')
        ax[1].set_title('JS Divergence with Sliding Window')
